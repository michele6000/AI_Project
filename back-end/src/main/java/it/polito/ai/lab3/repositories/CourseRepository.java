...
public interface CourseRepository extends JpaRepository<Course, String> {
  ...
  @Query(
    "SELECT s FROM Student s INNER JOIN s.courses c WHERE c.name=:courseName AND " +
    "s NOT IN (SELECT s FROM Student s INNER JOIN s.teams t INNER JOIN t.course c WHERE c.name=:courseName)"
  )
